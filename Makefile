.PHONY: run run-local-latte build clean dist local-latte release-test release

run: build
	@set -a; [ -f .env ] && . ./.env; set +a; ./build/install/brief/bin/brief

run-local-latte:
	@cd ../latte-v2 && mvn -pl latte-tui -am install
	@LATTE_LOCAL=true $(MAKE) run
